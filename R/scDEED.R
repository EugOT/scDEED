...
Distances.tSNE <- function(pbmc, pbmc.permuted, K, perplexity_score = 40, pre_embedding = "pca", check_duplicates = T, rerun = T) {
  ...
  distances <- distances::distances
  if (rerun) {
    pbmc <- Seurat::RunTSNE(pbmc, seed.use = reseed, perplexity = perplexity_score, reduction = pre_embedding, do.fast = T, check_duplicates = check_duplicates)
  }
  tSNE_distances <- distances(pbmc@reductions$tsne@cell.embeddings)

  pbmc.permuted <- Seurat::RunTSNE(pbmc.permuted, seed.use = reseed, perplexity = perplexity_score, reduction = pre_embedding, do.fast = T, check_duplicates = check_duplicates)
